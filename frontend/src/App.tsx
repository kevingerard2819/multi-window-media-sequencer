...
function App() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-mark"><span /><span /><span /></div><div><strong>SEQUENCE<span>STUDIO</span></strong><small>Multi-display control room</small></div></div><div className="topbar-right"><span className="connection"><i /> {error ? 'CONNECTION ISSUE' : 'SYSTEM ONLINE'}</span><span className="topbar-time">{new Date(tick).toLocaleTimeString()}</span></div></header>
    <section className="page-intro"><div><div className="eyebrow"><span className="eyebrow-line" /> LIVE PLAYBACK SYSTEM <span className="eyebrow-line" /></div><h1>Every display.<br /><em>In perfect sequence.</em></h1><p>Independent playlists, one synchronized moment. Manage every screen from a single control room.</p></div><div className="cycle-card"><small>CURRENT CYCLE</small><strong>{formatTime(currentCycle)}</strong><div className="cycle-track"><div style={{ width: `${state ? (currentCycle / state.cycleMs) * 100 : 0}%` }} /></div><span>OF 05:00:00 CYCLE</span></div></section>
